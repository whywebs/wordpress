        ...
        foreach ( $metabox['fields'] as $value ) {
        	...
        	switch( $value['type'] ) {
                ...
                case 'datetime':
                    if ( $advanced_field ) {
                        $date_value_time = '';
                        if ( isset($value['format']) && ! empty($option_value) ) {
                            $date_value_time = date($value['format'],$option_value);
                        }
                        if ( ! isset($value['timepicker']) || empty($value['timepicker']) ) {
                            $value['timepicker'] = "0";
                        }
                        if ( ! isset($value['datepicker']) || empty($value['datepicker']) ) {
                            $value['datepicker'] = "0";
                        }
						...
						name="'.esc_attr( $value['id'] ).'"
						id="'.esc_attr( $value['id'] ).'"
						value="'. esc_attr( $date_value_time ).'"
                        data-timepicker="' . esc_attr($value['timepicker']) . '"
                        data-datepicker="' . esc_attr($value['datepicker']) . '"
                        data-format="' . wp_kses_post($value['format']) . '"
						autocomplete="off">';
                        $output .= $option_wrap_end;
                    } else {
                    ...
                    if ( $advanced_field ) {
                        ...
                        $output .= '
                            <a href="#" class="kopa-framework-gallery-config button button-secondary">
                                ' . esc_html__('Config', 'kopa-framework') . '
                            </a>
                        ';
                        $output .= '</div>';
                        $output .= $option_wrap_end;
                ...
                case 'icon':
                    if ( $advanced_field ) {
                        $output .= $option_wrap_start;
                        $output .= '<div class="kf-row">';
                        $output .= '<div class="kf-col-xs-12">';
                        $output .= '<a class="kf-icon-picker" href="#">' . esc_html__('Select icon', 'kopa-framework') . '</a>';
                        $output .= '
                                <input type="hidden"
                                    name="'.esc_attr( $value['id'] ).'"
                                    id="'.esc_attr( $value['id'] ).'"
                                    value="'.esc_attr( $option_value ).'"
                                    autocomplete="off"
                                    class="large-text kf-icon-picker-value"/>
                            ';

                        $output .= '<span class="kf-icon-picker-preview"><i class="'.esc_attr( $option_value ).'"></i></span>';
