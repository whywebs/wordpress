...
class PSC_Product_Save_Data {
        ...
        if (array_key_exists('psc_variable_product_count', $product_data)) {
            for ($i = 0; $i <= $product_data['psc_variable_product_count']; $i++) {
                ...
                if ($this->insert_variable_product_status($product_data, $i)) {
                    ...
                    $array_manage['psc_variable_product_name' . $i] = $product_data['psc_variable_product_name' . $i];
                    $array_manage['psc_variable_product_regular_price' . $i] = $product_data['psc_variable_product_regular_price' . $i];
                    $array_manage['psc_variable_product_sale_price' . $i] = $product_data['psc_variable_product_sale_price' . $i];
                    $array_manage['_psc_manage_stock_variable' . $i] = ($product_data['_psc_manage_stock_variable' . $i]) ? $product_data['_psc_manage_stock_variable' . $i] : 0;
                    $array_manage['psc_variable_product_stock' . $i] = $product_data['psc_variable_product_stock' . $i];
                    $array_manage['psc_variable_product_stock_status' . $i] = ($product_data['psc_variable_product_stock_status' . $i]) ? $product_data['psc_variable_product_stock_status' . $i] : 0;
    ...
    public function insert_variable_product_status($product_data, $i) {
        ...
        if ((isset($product_data['psc_variable_product_name' . $i])) && !empty($product_data['psc_variable_product_name' . $i])) {
            if ((isset($product_data['psc_variable_product_sale_price' . $i])) && !empty($product_data['psc_variable_product_sale_price' . $i])) {
                $result = true;
            }
        }
